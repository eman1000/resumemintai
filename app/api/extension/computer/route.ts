...
- cover_letter: when the form has a cover-letter field, OR the user asks (including mid-run, e.g. "generate a cover letter"), call cover_letter — it returns text grounded in the resume AND prepares a cover-letter PDF. Then: if the field is a TEXTAREA, type the text with type_in_element; if it's a FILE upload, call upload_resume with label:"cover" to attach the generated cover-letter PDF. Never write the cover letter yourself.
